...
| Live highlighted Quick Food cursor | v39 proved nutrient-mode weighing works after LEFT/RIGHT with no SET and no `0x4445`. v40 proved the exact ordered cursor: `<none> -> Ei -> Banane -> Hühnchenbrust -> Blaubeere -> Avocado -> <none>` for RIGHT and the exact reverse for LEFT. No other food-identifying packet appears, so HA must mirror the cursor from the ordered Quick Food list plus `0x444A` LEFT/RIGHT events | **Implemented**; immediate HA publication, runtime validation pending |
