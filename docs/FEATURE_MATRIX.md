...
| SET-confirmed food report `0x4445` | Proven live for five physical Quick Foods, including UTF-8 `Hühnchenbrust`; captured vectors parse correctly. End-to-end HA runtime proof observed physical LEFT/RIGHT + SET transitions `unavailable -> Blaubeere -> Hühnchenbrust` as distinct Recorder states with the immediate-publication fix loaded. Raw capture shows `0x4445` arriving at the same timestamp as the SET `0x444A` event, not on LEFT/RIGHT navigation | Implemented sensor/state for the **SET-confirmed** food report; live-proven end to end |
| Live highlighted Quick Food cursor | Device behavior proves nutrient calculation uses the food highlighted with LEFT/RIGHT even when SET is never pressed. The existing `0x4445` path therefore does not yet represent this live cursor. A no-SET navigation + weighing capture is required to determine whether another report exists or HA must mirror the cursor from Quick Food order plus LEFT/RIGHT events | **Not yet implemented** |
